...
export default function MyListingsPage() {
  ...
  const handleSearch = async () => {
    try {
      ...
      const data = await response.json()
      // Filter the search results to only include the user's cars
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      >
        <h1 className="text-3xl font-bold mb-6">My Listed Cars</h1>
        ...
        {cars.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-xl text-muted-foreground">You haven't listed any cars yet.</p>
            <Button className="mt-4" onClick={() => router.push('/cars/new')}>
              List Your First Car
            </Button>
          </div>
        ) : (
